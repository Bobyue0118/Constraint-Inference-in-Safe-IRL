...
class PolicyIterationLagrange(ABC):
    ...
    def __init__(self,
        ...
        # print("self.fix_seed(self.seed)",self.seed)
    ...
    def learn(self,
        ...
        # for i in range(7):
        #     for j in range(7):
        #         print(cost_function(np.array([[i,j]]),[0]))
        # input('see learn')
    ...
    def learn_target(self,
        ...
        # input("transition_prob")
        self.reward_mat = 2*self.reward_mat #2,7,7,15
        # print(self.reward_mat)
        # input("self.reward_mat")
        policy_stable, dual_stable = False, False
        iter = 0
        for iter in tqdm(range(total_timesteps)):
            print('\npolicy_stable',policy_stable,dual_stable,'\n')
            ...
            self.num_timesteps += 1
            # Run the policy evaluation
            self.policy_evaluation_target(cost_function, transition)
            #print('self.v_m1',np.round(self.v_m,2))
            # input('self.v_m1')
            # Run the policy improvement algorithm
            policy_stable = self.policy_improvement_target(cost_function, transition)
            cumu_reward, length, dual_stable, traj = self.dual_update_target(cost_function)
        logger.record("train/iterations", iter)
        logger.record("train/cumulative rewards", cumu_reward)
        logger.record("train/length", length)
        # for i in range(7):
    ...
    def learn_irc(self,
              ...
              callback=None, ):
        #self.env_for_us = env_for_us
        # self.reward_mat += reward_correction
        ...
        for iter in tqdm(range(total_timesteps)):
            ...
            self.num_timesteps += 1
            # Run the policy evaluation
            self.policy_evaluation_irc(cost_function, transition, reward_correction)
            #print('self.v_m1',np.round(self.v_m,2))
            # input('self.v_m1')
            # Run the policy improvement algorithm
            policy_stable = self.policy_improvement_irc(cost_function, transition, reward_correction)
            cumu_reward, length, dual_stable, traj = self.dual_update_irc(cost_function)
        logger.record("train/iterations", iter)
        logger.record("train/cumulative rewards", cumu_reward)
        logger.record("train/length", length)
        # for i in range(7):
    ...
    def learn_irc_target(self,
        ...
        #self.env_for_us = env_for_us
        # self.reward_mat += reward_correction
        self.reward_mat = 1 * self.reward_mat # 2,7,7,15
    ...
    def dual_update_target(self, cost_function):
        """policy rollout for recording training performance"""
        obs = self.env.reset()
        cumu_reward, length = 0, 0
        actions_game, obs_game, costs_game = [], [], []
    ...
    def dual_update_irc(self, cost_function):
        """policy rollout for recording training performance"""
        obs = self.env.reset()
        cumu_reward, length = 0, 0
        actions_game, obs_game, costs_game = [], [], []
        while True:
            ...
            actions_game.append(actions[0])
            obs_primes, rewards, dones, infos = self.step(actions)
            # rewards[0] = self.reward_mat[obs_primes[0][0]][obs_primes[0][1]]
            # print('rewards', rewards)
            if type(cost_function) is str:
                costs = np.array([info.get(cost_function, 0) for info in infos])
                #print('costs',costs)
    ...
    def policy_evaluation(self, cost_function, transition):
        ...
        while delta >= self.stopping_threshold and iter <= self.max_iter-1:
            ...
            iter += 1
        # print(np.round(self.v_m,2))
        print("\n\nThe Policy Evaluation algorithm converged after {} iterations".format(iter),
              file=self.log_file)

    def policy_evaluation_target(self, cost_function, transition):
        iter = 0
        ...
        while delta >= self.stopping_threshold and iter <= self.max_iter-1:
            ...
            for x in range(self.height):
                for y in range(self.width):
                    # Run one iteration of the Bellman update rule for the value function
                    self.bellman_update_target(old_v, x, y, cost_function, transition)
                    # Compute difference
                    delta = max(delta, abs(old_v[x, y] - self.v_m[x, y]))
            iter += 1
        # print(np.round(self.v_m,2))
    ...
    def policy_evaluation_irc(self, cost_function, transition, reward_correction):
        iter = 0
        ...
        while delta >= self.stopping_threshold and iter <= self.max_iter-1:
            ...
            for x in range(self.height):
                for y in range(self.width):
                    # Run one iteration of the Bellman update rule for the value function
                    self.bellman_update_irc(old_v, x, y, cost_function, transition, reward_correction)
                    # Compute difference
                    delta = max(delta, abs(old_v[x, y] - self.v_m[x, y]))
            iter += 1
        # print(np.round(self.v_m,2))
    ...
    def policy_improvement_target(self, cost_function, transition):
        """Applies the Policy Improvement step."""
        policy_stable = True
        # Iterate states
        for x in range(self.height):
            for y in range(self.width):
                ...
                for action in range(self.n_actions):
                    states = self.env.reset_with_values(info_dicts=[{'states': [x, y]}])
                    # self.reward_mat = self.reward_mat * 2
                    # print(self.reward_mat)
                    # input('self.reward_mat_improvement')
                    assert states[0][0] == x and states[0][1] == y
                    # Compute next state, but not the same
                    # if the action is invalid, continue
                    next_state = [x+self.neighbors[action][0], y+self.neighbors[action][1]]
    ...
    def policy_improvement_irc(self, cost_function, transition, reward_correction):
        """Applies the Policy Improvement step."""
        policy_stable = True
        # Iterate states
        for x in range(self.height):
            for y in range(self.width):
                ...
                for action in range(self.n_actions):
                    ...
                    for i in range(len(x_coordinate)):
                        s_primes = [[x_coordinate[i], y_coordinate[i]]]
                        rewards = [self.reward_mat[s_primes[0][0]][s_primes[0][1]]]
                        rewards += reward_correction[x][y][action]
                        costs = cost_function(np.array(s_primes), [action])
    ...
    def bellman_update(self, old_v, x, y, cost_function, transition):
        ...
        for action in range(self.n_actions):
            ...
            for i in range(len(x_coordinate)):
                ...
                current_penalty = self.dual.nu().item()
                lag_costs = self.apply_lag * current_penalty * orig_costs[0]
                # print('s_primes,costs',s_primes,costs)
                # if lag_costs!=0:
                #     input('lag_costs!=0')
    ...
    def bellman_update_target(self, old_v, x, y, cost_function, transition):
        if [x, y] in self.terminal_states:
            return
        total = 0
        for action in range(self.n_actions):
    ...
    def bellman_update_irc(self, old_v, x, y, cost_function, transition, reward_correction):
        if [x, y] in self.terminal_states:
            return
        total = 0
        for action in range(self.n_actions):
    ...
    def predict_random(self, obs, state, deterministic=None):
        ...
        print('policy_prob',obs[0][0],obs[0][1],policy_prob,self.admissible_actions)
        input('Enter')
        action = random.choice([i for i in range(len(policy_prob)) if policy_prob[i]>0 or policy_prob[i]==0])
        print('action',action)
        return np.asarray([action]), state
