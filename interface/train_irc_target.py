...
from utils.data_utils import read_args_target, load_config, ProgressBarManager, del_and_make, load_expert_data, \
    load_config_target, \
    get_input_features_dim, process_memory, print_resource
from utils.model_utils import load_ppo_config, load_policy_iteration_config, get_hoeffding_ci_us, \
    get_hoeffding_ci_greedy, cal_discounted_cumulative_rewards_target, cal_discounted_cumulative_costs
...
def train(config):
    ...
    train_env_target, env_configs_target = make_train_env(env_id=config_target['env']['train_env_id'],
                                                          ...
                                                          normalize_reward=not config_target['env'][
                                                              'dont_normalize_reward'],
                                                          normalize_cost=not config_target['env'][
                                                              'dont_normalize_cost'],
                                                          ...
                                                          part_data=partial_data,
                                                          log_file=log_file,
                                                          noise_mean=config_target['env']['noise_mean'] if 'Noise' in
                                                                                                           config_target[
                                                                                                               'env'][
                                                                                                               'train_env_id'] else None,
                                                          noise_std=config_target['env']['noise_std'] if 'Noise' in
                                                                                                         config_target[
                                                                                                             'env'][
                                                                                                             'train_env_id'] else None,
                                                          max_scene_per_env=config_target['env']['max_scene_per_env']
                                                          if 'max_scene_per_env' in config_target[
                                                              'env'].keys() else None,
                                                          )
    ...
    while vareps_itr > vareps:
        ...
        if itra == 1:
            ...
            # print(np.round(expert_value_function,3))
            # input('expert_value_function')
            expert_policy = deepcopy(optimal_policy_with_true_constraint)
            if env_configs['unsafe_states'] != [[2, 0], [2, 1], [2, 2], [2, 3], [2, 4], [4, 2], [4, 3], [4, 4], [4, 5],
                                                [4, 6]]:
                for unsafe_state in env_configs['unsafe_states']:
                    # print('unsafe_state', unsafe_state)
                    # The expert policy generated by Lag-PPO needs modification
                    expert_policy[unsafe_state[0]][unsafe_state[1]] = expert_policy_unsafe[unsafe_state[0]][
                        unsafe_state[1]]
            else:
            ...
            expert_value_function_for_irc = deepcopy(expert_value_function)
            # input('itr:1')
        ...
        if itra >= 2:
            ...
            estimated_reward_correction = constraint_net.cal_reward_correction(expert_policy, gamma,
                                                                               estimated_transition,
                                                                               expert_value_function)
            # constraint_net.cal_reward_correction(expert_policy, gamma, estimated_transition, expert_value_function)
            if itra in plot_itra:
                constraint_visualization_2d(cost_function=constraint_net.reward_correction_function,
                                            feature_range=config['env']["visualize_info_ranges"],
                                            select_dims=config['env']["record_info_input_dims"],
                                            num_points_per_feature=env_configs['map_height'],
                                            obs_dim=train_env.observation_space.shape[0],
            ...
            for cnt in range(1):
                rewards_tmp = []
                costs_tmp = []
                nominal_agent6 = create_nominal_agent()
                with ProgressBarManager(forward_timesteps) as callback:
                    _, traj = nominal_agent6.learn_irc_target(
                        reward_correction=estimated_reward_correction,
                        total_timesteps=forward_timesteps,
                        cost_function=constraint_net.cost_function_zero,  # zero cost when implementing irc
                        transition=transition_target,  # estimated_transition,
                        callback=[callback] + all_callbacks
                    )
                    forward_metrics = logger.Logger.CURRENT.name_to_value
                    timesteps += nominal_agent6.num_timesteps
                rewards_tmp.append(np.round(
                    cal_discounted_cumulative_rewards_target(traj=traj, reward_states=env_configs['reward_states'],
                                                      gamma=gamma,final_reward=15),
                    5))
                costs_tmp.append(np.round(
                    cal_discounted_cumulative_costs(traj=traj, unsafe_states=env_configs['unsafe_states'], gamma=gamma),
                    5))
            rewards_tmp = np.array(rewards_tmp)
            rewards_agent_target.append(np.max(rewards_tmp[np.where(costs_tmp == np.min(np.array(costs_tmp)))]))
            costs_agent_target.append(np.min(np.array(costs_tmp)))
            # input('#########################\nend')

        ci = get_hoeffding_ci_us(height=env_configs['map_height'], width=env_configs['map_width'],
                                 n_actions=env_configs['n_actions'], sample_count=sample_count,
                                 v_m=expert_value_function, zeta_max=config['iteration']['zeta_max'],
                                 gamma=config['iteration']['gamma'], epsilon=config['iteration']['epsilon'], delta=0.1)
