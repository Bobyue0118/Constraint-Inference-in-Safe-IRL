...
from utils.data_utils import read_args, load_config_target, ProgressBarManager, del_and_make, load_expert_data, read_args_target,\
    get_input_features_dim, process_memory, print_resource
from utils.model_utils import load_ppo_config, load_policy_iteration_config, get_hoeffding_ci_us, get_hoeffding_ci_greedy, cal_discounted_cumulative_rewards_target, cal_discounted_cumulative_costs
...
def train(config):
    ...
    # init uniform sampling target env
    ...
    if 'PPO' in config.keys():
        ...
        iteration_parameters_target = load_policy_iteration_config(config=config_target,
                                                            env_configs=env_configs_target,
                                                            train_env=train_env_target,
                                                            seed=seed,
                                                            log_file=log_file)
        create_nominal_agent_target = lambda: PolicyIterationLagrange(**iteration_parameters_target)
	...
	# uniform sampling
        ...
        # print('transition', np.round(transition,3), 'transition_target', np.round(transition_target,3))
        # input('transition')
        ...
        if itra >= 2:
            ...
            #     nominal_agent = create_nominal_agent()
            #     with ProgressBarManager(forward_timesteps) as callback:
            #         _, traj = nominal_agent.learn_target(
            #             total_timesteps=forward_timesteps,
            ...
            for cnt in range(1):
                ...
                with ProgressBarManager(forward_timesteps) as callback:
                    _, traj = nominal_agent6.learn_target(
                        total_timesteps=forward_timesteps,
                        cost_function=constraint_net.cost_function,  # Cost should come from cost wrapper
                        transition=transition_target,
                        callback=[callback] + all_callbacks
                ...
                rewards_tmp.append(np.round(
                    cal_discounted_cumulative_rewards_target(traj=traj, reward_states=env_configs['reward_states'], gamma=gamma, final_reward=15),
                    5))
                costs_tmp.append(np.round(
                    cal_discounted_cumulative_costs(traj=traj, unsafe_states=env_configs['unsafe_states'], gamma=gamma), 5))
            rewards_tmp = np.array(rewards_tmp)*1
            rewards_agent.append(np.max(rewards_tmp[np.where(costs_tmp == np.min(np.array(costs_tmp)))]))
            costs_agent.append(np.min(np.array(costs_tmp)))
    ...
    with ProgressBarManager(forward_timesteps) as callback:
        ...
        #print('sample_count', sample_count)
        #input('itr:2')
    # print('rewards and costs:')
    print('target rewards:', rewards_agent)
    print('target costs:', costs_agent)
