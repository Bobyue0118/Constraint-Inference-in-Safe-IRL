...
def train(config):
	...
	# uniform sampling
        ...
        if itra >= 2:
            ...
            estimated_reward_correction = constraint_net.cal_reward_correction(expert_policy, gamma, estimated_transition, expert_value_function)
            ...
            for cnt in range(1):
                ...
                nominal_agent = create_nominal_agent()
                with ProgressBarManager(forward_timesteps) as callback:
                    _, traj = nominal_agent.learn_irc(
                        ...
                        transition=transition,# estimated_transition,
                        callback=[callback] + all_callbacks
                    )
                    forward_metrics = logger.Logger.CURRENT.name_to_value
                    timesteps += nominal_agent.num_timesteps
