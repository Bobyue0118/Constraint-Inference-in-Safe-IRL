...
def load_config_target(args=None):
    assert os.path.exists(args.config_file), "Invalid configs file {0}".format(args.config_file)
    assert os.path.exists(args.config_file_target), "Invalid configs file {0}".format(args.config_file_target)
    with open(args.config_file) as reader:
        config = yaml.safe_load(reader)
    with open(args.config_file_target) as reader_target:
        config_target = yaml.safe_load(reader_target)
    return config, config_target, args.DEBUG_MODE, args.LOG_FILE_PATH, args.PART_DATA, int(args.NUM_THREADS), int(args.SEED)
...
def read_args_target():
    parser = argparse.ArgumentParser()
    parser.add_argument("config_file", help="path to configs file")
    parser.add_argument("config_file_target", help="path to configs file target")
    # parser.add_argument("-t", "--train_flag", help="if training",
    #                     dest="TRAIN_FLAG",
    #                     default='1', required=False)
    parser.add_argument("-d", "--debug_mode", help="whether to use the debug mode",
